...
<div class="bg-white rounded-lg shadow-md border border-gray-200 max-w-2xl">
    <form action="{{ route('admin.students.update', $student->id) }}" method="POST">
        ...
        <div class="p-6 md:p-8">
            ...
            <!-- Validation Errors -->
            @if ($errors->any())
            <div class="mb-6 bg-red-50 border border-red-300 text-red-800 p-4 rounded-lg">
                <strong class="block font-medium">Validation failed:</strong>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                           value="{{ old('guardian_mobile_number', $student->guardian_mobile_number) }}"
                           required>
                </div>
                ...
                <div>
                    ...
                    <input type="text"
                           ...
                           value="{{ old('exam_year', $student->exam_year) }}">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="month"
                           ...
                           name="joining_month"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors"
                           value="{{ old('joining_month', $student->joining_month) }}"
                           required>
                </div>
                ...
                <div class="md:col-span-2 border-t border-gray-200 pt-6">
                    ...
                    <select id="batch_day_select"
                        ...
                        @foreach($batchDays as $bd)
                            <option value="{{ $bd->id }}" {{ old('batch_day_id', $student->batch_day_id) == $bd->id ? 'selected' : '' }}>
                                {{ $bd->days }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="batch_time_select"
                        ...
                        @foreach($batchTimes as $bt)
                            <option data-batch-day-id="{{ $bt->batch_day_id }}"
                                    value="{{ $bt->id }}"
                                    {{ old('batch_time_id', $student->batch_time_id) == $bt->id ? 'selected' : '' }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get the initial (or old) time ID to re-select after filtering
    const selectedTimeId = @json(old('batch_time_id', $student->batch_time_id));
    ...
    function filterTimes() {
        ...
        let isSelectedTimeVisible = false;
        ...
        timeOptions.forEach(opt => {
            ...
            if (!selectedDay || dayId === selectedDay) {
                opt.style.display = '';
                if (opt.value == selectedTimeId) {
                    isSelectedTimeVisible = true;
                }
            } else {
        ...
        // Re-select the correct time
        if (isSelectedTimeVisible) {
            batchTimeSelect.value = selectedTimeId;
        } else {
            // If the previously selected time is no longer valid, reset to placeholder
